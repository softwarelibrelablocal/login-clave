...
class sspmod_saml_Message {
	...
	public static function checkSign(SimpleSAML_Configuration $srcMetadata, SAML2_SignedElement $element) {
		...
		SimpleSAML_Logger::debug('Comprobando firma electronica');
		/* Find the public key that should verify signatures by this entity. */
		$keys = $srcMetadata->getPublicKeys('signing', TRUE, 'validate.');
		if ($keys !== NULL) {
			SimpleSAML_Logger::debug('Comprobando firma electronica con ' . count($keys) . ' certificados');
			$pemKeys = array();
			foreach ($keys as $key) {
				switch ($key['type']) {
				...
				//SimpleSAML_Logger::debug('Cert encontrado: ' . $pemKeys[0] );
			}

		} elseif ($srcMetadata->hasValue('certFingerprint')) {
			$certFingerprint = $srcMetadata->getArrayizeString('certFingerprint');
			SimpleSAML_Logger::debug('Buscando por fingerprint : ' . $certFingerprint );
			...
			SimpleSAML_Logger::debug('Buscando por CA ' . $caFile);
			if ($caFile === NULL) {
