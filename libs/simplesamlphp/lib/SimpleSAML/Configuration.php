...
class SimpleSAML_Configuration {
	...
	public function getPublicKeys($use = NULL, $required = FALSE, $prefix = '') {
		...
		SimpleSAML_Logger::debug('Buscando claves publicas. Prefijo: ' . $prefix );

		if ($this->hasValue($prefix . 'keys')) {
			$ret = array();
			foreach ($this->getArray($prefix . 'keys') as $key) {
				if ($use !== NULL && isset($key[$use]) && !$key[$use]) {
					SimpleSAML_Logger::debug('Uso de clave no correspondiente: ' . $key[$use] );
					continue;
				}
				SimpleSAML_Logger::debug('Tipo de clave: ' . $key['X509Certificate'] );
			...
			if (!empty($ret)) {
				SimpleSAML_Logger::debug('Saliendo');
				return $ret;
			}
		} elseif ($this->hasValue($prefix . 'certData')) {
			$certData = $this->getString($prefix . 'certData');
			SimpleSAML_Logger::debug('Entrando por CertData: ' . $certData );
			...
			SimpleSAML_Logger::debug('Entrando por certificate: ' . $file );
