...
class SAML2_Assertion implements SAML2_SignedElement {
	...
	private function parseAttributes(DOMElement $xml) {
      ...
      if($attribute->hasAttribute('stork:AttributeStatus')) {
        $this->attributes[$name]['AttributeStatus'] = $attribute->getAttribute('stork:AttributeStatus');
      }

			$values = SAML2_Utils::xpQuery($attribute, './saml_assertion:AttributeValue');
			foreach ($values as $value) {
        //XXX: from [] -> ['AttributeValues']
				$this->attributes[$name]['AttributeValues'] = trim($value->textContent);
			}
		}
	}
